...
const UpdateBooksModal = ({
  ...
  // Set default values for subjects based on booksData
  useEffect(() => {
    if (booksData) {
      const updatedSubjectList = {};
      booksData.books.forEach((book, index) => {
        updatedSubjectList[`subject${index + 1}`] = book.subject || "";
      });
    ...
    defaultValues: {
      ...
      books: booksData
        ? booksData.books.map((book) => ({
            ...
            noBooksFirst: book.noBooksFirst || "",
            noBooksSecond: book.noBooksSecond || "",
            noBooksThird: book.noBooksThird || "",
            noBooksFourth: book.noBooksFourth || "",
            noBooksFifth: book.noBooksFifth || "",
            noBooksSixth: book.noBooksSixth || "",
            noBooksSeventh: book.noBooksSeventh || "",
          }))
        : [
            {
              subject: "",
  ...
  const onSubmit = async (data) => {
    ...
    await axios
      .put(`${apiKey}/user/staff/update-books-data`, updatedFormData, config)
      .then((res) => {
        if (res.data.successful) {
          toast.success("Required books updated successfully..!", {
            position: "bottom-right",
          });
          reset();
          closeModal();
    ...
    >
      <DialogPanel>
        <div className="flex justify-between">  
          <Title className="mb-8 text-primary">Update Required Books</Title>
          <FaXmark
