...
const UpdateTimetable = ({
  ...
  const [standard, setStandard] = useState(timetableData.standard);
  const subjects = [
    "Marathi",
    "English",
    "Science",
  ...
  const handleSubmit = async (data) => {
    ...
    await axios
      .put(`${apiKey}/admin/update-timetable`, data, config)
      .then((res) => {
  ...
  useEffect(() => {
    // Function to set default values for timetable
    const setDefaultValues = (timetableData) => {
      if (timetableData) {
        // Set standard
        setStandard(timetableData.standard);

        // Set values for Monday to Friday timetable
        Object.keys(timetableData.timetable).forEach((day) => {
          Object.keys(timetableData.timetable[day]).forEach((period) => {
            const subject = timetableData.timetable[day][period].subject;
            handleSubjectSelect(day, period, subject);
          });
        });

        // Set values for Saturday timetable
        Object.keys(timetableData.saturdayTimetable).forEach((day) => {
          Object.keys(timetableData.saturdayTimetable[day]).forEach(
            (period) => {
              const subject =
                timetableData.saturdayTimetable[day][period].subject;
              handleSaturdaySubjectSelect(day, period, subject);
            }
          );
        });
      }
    ...
    // Call the function to set default values when component mounts
    setDefaultValues(timetableData);
  }, [timetableData]);
      ...
      <DialogPanel>
        ...
        <div className="flex flex-col gap-4 px-4 w-full">
          <div className="flex justify-between">
            <TextInput
              className="w-fit"
              value={"Standard " + timetableData.standard}
              disabled
            />
            <Button
