...
import TransferBackStaffModal from "../../../modals/admin/TransferBackStaffModal/TransferBackStaffModal";
...
const TransferredStaff = () => {
  ...
  const [transferBackModalOpen, setTransferBackModalOpen] = useState(false);
  ...
  const fetchStaff = async () => {
    ...
    await axios
      .get(`${apiKey}/admin/get-transferred-staff`, config)
      .then((res) => {
        if (res.status === 200) {
          setData(res.data);
        }
  ...
  return (
    <div className="flex flex-col h-full justify-evenly items-center">
      <div className="w-full flex justify-between mb-8 font-opensans">
        ...
        <div className="flex gap-4">
          <CSVLink
            data={csvData}
            headers={headers}
            filename="schoolmate_transferred_staff.csv"
          >
            <button
              onClick={() => exportData()}
      ...
      {transferBackModalOpen && (
        <TransferBackStaffModal
          isOpen={transferBackModalOpen}
          fetchStaff={fetchStaff}
          closeModal={() => setTransferBackModalOpen(false)}
          userData={user}
        />
      )}
    </div>
...
export default TransferredStaff;
